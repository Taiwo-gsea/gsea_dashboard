...
async def calculate_sci(request: SCIRequest, db: AsyncSession = Depends(get_db)):
    ...
    persisted to the sci_measurements table so it is available to the
    Energy Trend and Comparative Analysis pages across the session.
    """
    try:
...
async def estimate_sci_from_proxy(request: ProxyMetricRequest, db: AsyncSession = Depends(get_db)):
    ...
    metric reading are persisted so historical proxy-based estimates
    are queryable in the same way as direct measurements.
    """
    try:
